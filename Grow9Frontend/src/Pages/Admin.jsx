...
const AdminPage = () => {
  ...
  const handleSponsorRegistration = async (e) => {
    ...
    try {
      const response = await fetch(
        baseURL + "/api/Admin/sponserregisteration",
        {
          method: "POST",
          headers: {
  ...
  const fetchSponsorCustomers = async (sponsorId) => {
    ...
    try {
      const response = await fetch(
        baseURL + `/api/sponser/customerlist/${sponsorId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("adminToken")}`,
  ...
  const fetchEarningsData = async () => {
    ...
    try {
      const response = await fetch(
        baseURL + `/api/admin/earnings/interval=${timeRange}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("adminToken")}`,
            ...
            {registrationResponse && (
              <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                ...
                <div className="space-y-2 text-green-700">
                  <p>
                    <strong>Sponsor ID (UUID):</strong>{" "}
                    {registrationResponse.sponsorId ||
                      registrationResponse.uuid}
                  </p>
                  <p>
                    <strong>User Name:</strong>{" "}
                    {registrationResponse.userName || registrationResponse.name}
                  </p>
                </div>
              </div>
            )}
                ...
                <table className="w-full">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {sponsors.map((sponsor) => (
                      <tr
                        key={sponsor.sponsorId || sponsor.id}
              ...
              <div className="border-t bg-gray-50 p-6">
                ...
                {customers.length > 0 ? (
                  ...
                  <p className="text-gray-600">
                    No customers found for this sponsor.
                  </p>
                )}
                    ...
                    <div className="mb-6">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            setTempPercentage(parseFloat(e.target.value) || 0)
                          }
                          min="0"
                          max="100"
                          step="1"
